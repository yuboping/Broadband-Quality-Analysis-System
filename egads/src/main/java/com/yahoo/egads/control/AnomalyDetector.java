...
public class AnomalyDetector {
    ...
    public AnomalyDetector(TimeSeries theMetric, long period, long firstTimeStamp)
            throws Exception {
        if (theMetric == null) {
    ...
    public Map<String, Object> tune(TimeSeries.DataSequence expectedValues,
        ...
        for (AnomalyDetectionModel model : models) {
            if (!isTuned.get(i)) {
                ...
                case "ExtremeLowDensityModel":
                    map = model.tuneThreshold(metric.data, expectedValues, anomalySequence);
                    break;
                default:
