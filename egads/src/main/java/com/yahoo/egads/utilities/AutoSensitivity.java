...
public class AutoSensitivity {
    static org.apache.logging.log4j.Logger logger = org.apache.logging.log4j.LogManager
            .getLogger(AutoSensitivity.class.getName());
    ...
    public static Float getLowDensitySensitivity(Float[] data, float sDAutoSensitivy,
        ...
        while (data.length > 0) {
            ...
            Float maxDelta = (float) sDAutoSensitivy
                    * StatsUtils.getSD(data, StatsUtils.getMean(data));

            logger.debug("AutoSensitivity: Adding: " + fData.get(0) + " SD: " + maxDelta);

            // Add points while it's in the same cluster or not part of the
            // other cluster.
            ...
            while (data.length > 0 && (centroid - data[0]) <= ((float) (maxDelta))) {
                ...
                if (data.length > 0) {
                    ...
                    logger.debug("AutoSensitivity: Adding: " + data[0] + " SD: " + maxDeltaInit
                            + " SD': " + maxDelta);
                }
            }
            if (data.length > 0) {
    ...
    public static Float[] getAdaptiveMaxMinSigmaSensitivity(Float[] data, float amntAutoSens,
            float k) {
        Arrays.sort(data);
        Float mean = StatsUtils.getMean(data);
        Float sd = StatsUtils.getSD(data, mean);
