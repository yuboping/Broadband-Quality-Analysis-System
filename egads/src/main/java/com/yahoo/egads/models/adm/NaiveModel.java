        ...
        for (int k = 0; k < n; k++) {
            ...
            for (int i = 0; i < 2; i++) {
                ...
                if (isAnomaly(errors, threshold) == true && actualAnomaly == true && anomaly == 1
                        && (isDetectionWindowPoint(maxHrsAgo, windowStart,
                                observedSeries.get(anomalyIndex).time, observedSeries.get(0).time)
                                || (maxHrsAgo == 0 && i == (n - 1)))) {
                    anomaly = 0;
                    logger.debug("TS:" + observedSeries.get(anomalyIndex).time + ",E:"
                            + arrayF2S(errors) + ",TH:" + arrayF2S(thresholdErrors) + ",OV:"
                            + observedSeries.get(anomalyIndex).value + ",EV:" + expected[i]);
                    output.add(new Interval(observedSeries.get(anomalyIndex).time, anomalyIndex,
                            errors, thresholdErrors, observed[i], expected[i],
