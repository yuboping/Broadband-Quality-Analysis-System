...
public class SimpleThresholdModel extends AnomalyDetectionAbstractModel {
    ...
    public void tune(DataSequence observedSeries, DataSequence expectedSeries,
        ...
        if (simpleThrType.equals("AdaptiveKSigmaSensitivity")) {
            thr = AutoSensitivity.getAdaptiveKSigmaSensitivity(observedSeries.getValues(),
                    amntAutoSensitivity);
        } else {
            thr = AutoSensitivity.getAdaptiveMaxMinSigmaSensitivity(observedSeries.getValues(),
                    amntAutoSensitivity, sDAutoSensitivity);
        }
    ...
    public IntervalSequence detect(DataSequence observedSeries, DataSequence expectedSeries)
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (((thr[0] != null && entry.value >= thr[0])
                    || (thr[1] != null && entry.value <= thr[1]))
                    && (isDetectionWindowPoint(maxHrsAgo, windowStart, entry.time,
