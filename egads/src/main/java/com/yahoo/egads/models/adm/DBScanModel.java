            ...
            for (IdentifiedDoublePoint p : c.getPoints()) {
                ...
                if (observedSeries.get(p.getId()).value != expectedSeries.get(p.getId()).value
                        && (isDetectionWindowPoint(maxHrsAgo, windowStart,
                                observedSeries.get(p.getId()).time, observedSeries.get(0).time)
                                || (maxHrsAgo == 0 && p.getId() == (n - 1)))) {
                    output.add(new Interval(observedSeries.get(p.getId()).time, p.getId(), errors,
                            thresholdErrors, observedSeries.get(p.getId()).value,
