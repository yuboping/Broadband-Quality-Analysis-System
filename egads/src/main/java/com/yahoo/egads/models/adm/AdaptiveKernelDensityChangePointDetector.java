...
/**
 * AdaptiveKernelDensityChangePointDetector implements density-based algorithm
 * for change point detection.
 * 
 * Input: 1. the residual time-series = actual - expected 2. PRE_WINDOW_SIZE:
 * the size of PRE_WINDOW 3. POST_WINDOW_SIZE: the size of POST_WINDOW 4.
 * CONFIDENCE: the confidence level at which the threshold for the KL-Divergence
 * score is computed.
 * 
 * Output: the time indices when the distribution of residual changes
 * significantly
 * 
 * In particular, the algorithm slides two side-by-side windows (PRE_WINDOW and
 * POST_WINDOW) and computes the KL-divergence between the distribution of the
 * residuals in two windows for each time index. The distribution of the
 * residuals in each window is computed non-parametrically using Kernel Density
 * Estimation with Gaussian kernels with adaptive bandwidths.
 * 
 * Once the KL-divergence is computed for each time index, it is thresholded
 * (where the threshold is directly proportional to 'CONFIDENCE'). And finally
 * for each continuous segment of the thresholded KL-divergence time-series, the
 * index of the maximum value is reported as a change point.
 * 
 * @author amizadeh
 *
 */
...
public class AdaptiveKernelDensityChangePointDetector extends AnomalyDetectionAbstractModel {
    ...
    public IntervalSequence detect(DataSequence observedSeries, DataSequence expectedSeries)
        ...
        if (observedSeries.size() != expectedSeries.size()) {
            throw new Exception(
                    "The observed time-series must have the same length as the expected time-series.");
        }
        ...
        ArrayList<Integer> changePoints = detectChangePoints(residuals, this.preWindowSize,
                this.postWindowSize, this.confidence);
        ...
        if (this.outputDest.equals("STD_OUT_ALL")) {
            ...
            for (int i = 0; i < n; ++i) {
                ...
                logger.debug("TS:" + observedSeries.get(i).time + ",SC:"
                        + String.join(":", arrayF2S(new Float[] { score[i] })) + ",LV:"
                        + arrayF2S(new Float[] { level[i] }) + ",OV:" + observedSeries.get(i).value
                        + ",EV:" + expectedSeries.get(i).value);

                result.add(new Interval(observedSeries.get(i).time, i, new Float[] { score[i] },
                        new Float[] { level[i] }, observedSeries.get(i).value,
                        expectedSeries.get(i).value, (isCP)));
            }
        } else {
            for (int index : changePoints) {
                if (isDetectionWindowPoint(maxHrsAgo, windowStart, observedSeries.get(index).time,
                        observedSeries.get(0).time)) {
                    result.add(new Interval(observedSeries.get(index).time, index,
                            new Float[] { score[index] }, new Float[] { level[index] },
                            observedSeries.get(index).value, expectedSeries.get(index).value));
                }
            }
        }
    ...
    public ArrayList<Integer> detectChangePoints(float[] residuals, int preWindowSize,
            int postWindowSize, float confidence) {
    ...
    protected float[] computeKLScore(float residual, int preWindowSize, int postWindowSize,
        ...
        if (len < preWindowSize) {
        ...
        } else if (len < (preWindowSize + postWindowSize)) { // Filling the
                                                             // post-window
            ...
            buffer.addLast(residual);
            preKernelSum.addLast(
                    ListUtils.kernelSubSum(residual, buffer, sdBuffer, 0, preWindowSize - 1));
            ...
            LinkedList<Float> postDensity = ListUtils.maxQ(
                    postKernelSum.subList(preWindowSize, preWindowSize + postWindowSize), eps);
            ...
            levelThreshold = (float) (-Math.log(levelSet) - Math.log(2 * Math.PI) / 2
                    - ListUtils
                            .sumLog(sdBuffer.subList(preWindowSize, preWindowSize + postWindowSize))
                            / postWindowSize);
    ...
    @Override
    public Map<String, Object> tuneThreshold(DataSequence observedSeries,
            DataSequence expectedSeries, IntervalSequence anomalySequence) throws Exception {
