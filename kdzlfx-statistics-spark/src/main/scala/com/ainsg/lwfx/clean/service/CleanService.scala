  ...
  def clean(): Unit = {
    ...
    try {
      for (business <- arrayBusiness) {
        ...
        localDir = CleanConf.hadoopMaster + CleanConf.getProp("clean.business." + business + ".localDir")
        newFileDir = CleanConf.hadoopMaster + CleanConf.getProp("clean.business." + business + ".newFileDir")
