...
import com.ainsg.lwfx.util.ToolsUtil
...
object TerminalService {
  ...
  def terminalStatistic(date: String, citycode: String, startDay: String): Unit = {
    ...
    var userTmpFile = new File(userTmpDir)
    FileUtil.deleteDir(userTmpFile)
    
    //话单数据处理 存在地市目录
//    userdetStatistic(date, month, citycode)
    userdetStatistic("20191007", "201910", citycode)
    //认证失败数据处理
//    userauthfailStatistic(date, month)
    userauthfailStatistic("20191015", "201910")
    //数据汇总、去重
    dealAllData("201910")
    //入库
    loadDB(startDay)
  ...
  def loadDB(startDay: String): Unit = {
    ...
    var companyViewName = "company_bq_" + month
    loadData(TerminalConf.terminalUserDir + "/" + startDay, companyViewName)
    var sql = "select USERNAME,AREANO,Organization_Name,AUTHFAIL_FLAG,SHORTTIME_FLAG,OFTENDOWN_FLAG from " + companyViewName
    LOG.info("最终入COMPANY_USER_BQ_" + month + "库, SQL：" + sql)
    val bqData = spark.sql(sql)
    bqData.show()
    spark.catalog.dropTempView(companyViewName)
   ...
   * 话单数据处理
   */
  def userdetStatistic(date: String, month: String, citycode: String): Unit = {
    var keyname = TerminalConf.userdet_name
    var dataDir = CleanConf.getProp("clean.business."+keyname+".newFileDir") + "/" + month
    var isCity = CleanConf.getProp("clean.business."+keyname+".isCity")
    if("1".equals(isCity)) {
      var citys = FileUtil.getDirNameFromPath(dataDir);
      ...
      if(!citys.isEmpty) {
        //加载数据
        citys.foreach(code => {
          processUserDet(date, month,dataDir, code)
        })
    }
    } else {
      processUserDet(date, month,dataDir, "")
    }
    
  }
  
  def processUserDet(date: String, month: String, dataDir: String, citycode: String): Unit = {
    var viewName = "user_date_" + citycode
    var sourceDir = dataDir
    if(!"".equals(citycode)) {
      sourceDir = dataDir + "/" + citycode
    }
    if(!FileUtil.isExist(sourceDir)) {
      LOG.error("目录:"+sourceDir+"不存在 is not exist")
      return
    }
    var data = loadData(sourceDir, viewName)
    /** 数据清洗： 去除正常下线数据 ,补齐下次上线时间字段 */
    cleanUserDet(date, citycode, viewName)
    /** 频繁掉线终端  */
...
//      usageBPPP.show()
  ...
  def cleanUserDet(date: String, citycode: String, viewName: String): Unit = {
    var sql = "select AREANO,USERNAME,DOWNREASON,MACK," +
              ...
              "AND substr(t1.STARTTIME,0,8)='"+date+"'"
  ...
  def oftenDown(citycode: String, viewName: String): DataFrame = {
    var sql = "select USERNAME,AREANO,MACK,(CASE WHEN COUNT(*) > 5 THEN 1 ELSE 0 END) AS OFTENDOWN_FLAG,COUNT(*) AS OFTENDOWN_COUNT from " + viewName + " group by USERNAME,AREANO,MACK" // > 10
  ...
  def shortTimeDown(citycode: String, viewName: String): DataFrame = {
    var sql = "select USERNAME,AREANO,(CASE WHEN COUNT(*) > 3 THEN 1 ELSE 0 END) AS SHORTTIME_FLAG,COUNT(*) AS SHORTTIME_COUNT from " + viewName +
  ...
  def userauthfailStatistic(date: String, month: String): Unit = {
    ...
    var sourceDir = CleanConf.getProp("clean.business."+keyname+".newFileDir") + "/" + month
    if(!FileUtil.isExist(sourceDir)) {
      LOG.error("目录:"+sourceDir+"不存在 is not exist")
      return
    }
    ...
    rtData.show()
    // 用户信息数据
    sql = "select t2.USERNAME,t2.AREANO,(CASE WHEN t1.AUTHFAIL_COUNT > 5 THEN 1 ELSE 0 END) AS AUTHFAIL_FLAG,t1.AUTHFAIL_COUNT from " + viewName + " t1 join " + userTable + " t2 on t1.USERNAME = t2.USERNAME" 
...
//      return
//    }
    var sourceDir = TerminalConf.terminalUserTmpDir + "/" + month + "/authfail"
...
//      return
//    }
    var sql = "select t1.USERNAME,t1.AREANO,t3.Organization_Name,0 AS BADQUALITY,IFNULL(t2.AUTHFAIL_FLAG,'0') AS AUTHFAIL_FLAG,IFNULL(t2.AUTHFAIL_COUNT,'0') AS AUTHFAIL_COUNT,t1.SHORTTIME_FLAG,t1.SHORTTIME_COUNT,t1.OFTENDOWN_FLAG,t1.OFTENDOWN_COUNT from " + 
...
//      var insertSql = "INSERT INTO CES_TERMINAL_CHARACTER_BQ(USERNAME,AREANO,AUTHFAIL_FLAG,AUTHFAIL_COUNT,SHORTTIME_FLAG,SHORTTIME_COUNT,OFTENDOWN_FLAG,OFTENDOWN_COUNT) VALUES (?,?,?,?,?,?,?,?)"
